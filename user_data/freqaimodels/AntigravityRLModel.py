...
class AntigravityRLModel(ReinforcementLearner):
    """
        ...
        def _calculate_reward_raw(self, action: int) -> float:  # noqa: C901
            ...
            if self._position == Positions.Long:
                ...
                if action == Actions.Long_exit.value:
                    if pnl > self._FEE_THRESHOLD:
                        multiplier = 1.5 if trade_duration <= max_trade_duration else 0.5
                        return float(pnl * factor * multiplier * 5.0)

                    if self._LOSS_THRESHOLD < pnl < 0:
                        return 0.3  # reward cutting a loss early

                    if 0 <= pnl <= self._FEE_THRESHOLD:
                        return -0.1  # penalise churn (exit with no gain)

                    return float(pnl * factor * 2.0)  # stopped out
